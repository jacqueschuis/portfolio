  ...
  return (
    <div className="rounded-3xl flex flex-col justify-between shadow-lg box-border p-6 mb-5 break-inside-avoid-column">
      <div className="flex justify-between items-center">
        <h3
          onClick={handleClick}
          className="w-full cursor-pointer transition-all text-2xl font-bold hover:text-blue-light"
        >
          {degree} in {subject}
        </h3>
      </div>
      ...
      >
        ...
        <animated.div style={expand}>
          <p className="my-2 pl-5">
            <span className="font-bold">Specializations:</span> {specialization}
          </p>
          <ul className="list-inside list-disc mb-2">
            {description.map((entry, index) => (
              <li className="mb-1 pl-5" key={`${institution}Desc${index}`}>
                {entry}
              </li>
            ))}
          </ul>
          ...
          {specialEventDescription && (
            <ul className="list-inside list-disc">
              {specialEventDescription.map((entry, index) => (
                <li className="mb-1 pl-5" key={`${institution}Spec${index}`}>
                  {entry}
                </li>
              ))}
            </ul>
      ...
      <Section>
        ...
        <div className="container md:columns-2 gap-10 self-center text-blue-dark">
          {education.map((item, index) => (
            <EducationEntry key={`edu${index}`} {...item} />
          ))}
        </div>
      </Section>
    </Element>
